...
- `docs/sample-company-seed.json` — 회사 자산 9종만 (가벼운 락인 시드용)
- `docs/sample-company-full.json` — 자산 + 프로젝트 통째 (전체 백업·이전 양식)

외부 AI에 양식을 의뢰할 때 같이 첨부하면 형식 일치율이 더 높아집니다.

---

# 부록 A. 전체 데이터 양식 (`kind: "company-full"`)

신규 가입 회사가 **기존에 운영하던 프로젝트 몇 개를 통째로 옮겨와** 견적·마감재·일정·지출 자료를 참고하면서 시작하는 시나리오용 양식입니다.

⚠️ **현재 사이클**: `📦 전체 데이터 내보내기`(다운로드)만 가능합니다. `📥 가져오기`는 외래키 그래프 검증이 완료된 다음 사이클에 열립니다. 이번 사이클의 산출물은 **양식 명세 + 표준 회사 JSON** 까지입니다.

## A-1. 최상위 구조

`kind: "company-full"` 일 때 회사 자산 9종(위 본문)에 더해 다음이 추가됩니다.
...
```json
{
  ...
  ...
  "projects": [ ... ],   // ★ 추가됨
  "counts": { "projects": 12, "materials": 340, ... }
}
```
...
```json
{
  "id": "cln1a2b3c4",                 // 원본 cuid 보존(import 사이클에서 oldId→newId 매핑용)
  "name": "삼성동 OO아파트 1503호",
  "customerName": "홍길동",
  "customerPhone": "010-1234-5678",
  "siteAddress": "서울시 강남구 삼성동 ...",
  "contractAmount": "85000000",
  "startDate": "2026-04-01T00:00:00.000Z",
  "expectedEndDate": "2026-05-15T00:00:00.000Z",
  "status": "IN_PROGRESS",
  "area": "32.5",
  "siteCode": "2604_삼성동OO",
  "memo": "...",
  "createdById": null,                // ★ 사용자 FK 는 모두 null (보안)
  "createdAt": "...",
  "updatedAt": "...",
...
### (1) ID 보존
모든 row 의 원본 `id`(cuid) 가 그대로 보존됩니다. import 사이클에서는 다음과 같이 처리됩니다:
- import 시 새 cuid 재발급
- 같은 export 안의 다른 모델이 참조하는 ID 는 `oldId → newId` 매핑 테이블로 치환
- 예: `Material.purchaseOrderId="cln_old"` → import 후 `"cln_new"` 로 자동 매핑

### (2) 사용자 FK 마스킹
다음 필드는 export 시 **모두 `null`** 로 마스킹됩니다 (외부 회사로 사용자 정보 새지 않게):

| 필드명 | 의미 |
|--------|------|
| `createdById`, `updatedById` | 생성/수정자 |
| `authorId` | 보고서 작성자 |
| `uploaderId` | 사진 업로더 |
| `completedById` | 체크리스트 완료자 |
| `changedById` | 마감재 이력 변경자 |
| `byUserId` | 일정 변경 행위자 |
| `requestedById` | 자재 요청자 |
| `userId` (ProjectMember) | 멤버 |
| `correctedById` (ApplianceSpec) | 정정자 |

→ import 사이클에서는 임포트 실행 OWNER 의 id 로 일괄 매핑됩니다. "누가 만들었나" 정보는 외부에서 들어온 데이터에서는 손실됩니다.

### (3) Vendor 매핑 (예정)
import 시 `Expense.vendorId`, `PurchaseOrder.vendorId`, `DailyScheduleEntry.vendorId` 가 export 회사의 vendor id 를 가리킵니다. 이걸 신규 회사의 vendor id 와 연결하는 정책:
- **1단계**: 같은 export 안의 `vendors[]` 의 `(name, category)` 로 신규 회사 vendor 와 매칭
- **2단계**: 없으면 vendor 를 새로 생성(import 동시에)
- **3단계**: 그래도 매칭 안 되면 vendorId=null 로 두고 vendor 텍스트(`Expense.vendor` 자유 텍스트) 만 보존

### (4) Cloudinary 사진
`ProjectPhoto.url` 은 외부 호스팅 URL 이라 그대로 보존됩니다. 원본 회사가 사진을 지우면 신규 회사의 import 본도 깨지므로, 정식 출시 시점에 "import 시 Cloudinary 폴더 복사" 옵션 검토.

### (5) 회사 메타
`company` 섹션의 `rate*`, `hideExpenses` 도 export 됩니다. import 시 적용 여부는 사용자 옵션으로 (Seed 모드에서는 기본 무시).

## A-4. 외부 AI 에 의뢰할 때 팁

신규 가입 회사 OWNER 가 ChatGPT/Claude 에 다음과 같이 요청합니다:

> "이 문서(import-schema.md)와 샘플(sample-company-full.json)을 참고해서 우리 회사용 전체 데이터 JSON 을 만들어줘. 우리 회사 정보는 이렇고, 기존에 진행한 프로젝트 자료를 줄게: ..."

AI 가 회사마다 다른 양식의 엑셀·메모·견적서·통장 데이터를 우리 양식으로 변환해 JSON 을 뱉으면, 봉기님이 검토 → Suplex 에 업로드 → 신규 회사가 자기 자료가 채워진 상태로 시작합니다.

## A-5. 양식 두 가지의 선택 기준

| 시나리오 | 양식 |
|----------|------|
| 우리 표준 룰만 신규 회사에 시드하고 싶다 | `company-assets` |
| 기존 진행 프로젝트 2-3건도 같이 옮기고 싶다 | `company-full` |
| 같은 회사 안에서 백업/복구 | `company-full` |
| 회사 간 합병·분할 | `company-full` (정식 출시 시점 import) |
